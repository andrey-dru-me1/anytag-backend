...
- **Simple handlers** return `Result<_, (StatusCode, String)>`; **complex handlers** (e.g., users) return `Result<_, HandlerErr>` — both coexist in the same project
- **Error codes** must be added to the [`ErrCode`](../../src/handlers/mod.rs:19) enum with `#[derive(AsRefStr, Display)]` and `#[strum(serialize_all = "SCREAMING_SNAKE_CASE")]`
