...
- **Two error patterns exist**: tuple-based `(StatusCode, String)` for simple handlers and structured `HandlerErr` with `ErrCode` enum for complex handlers — questioning which one to use is expected
